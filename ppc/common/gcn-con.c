...
//#define FAKE_GCN_CON 1

#include "../include/system.h"

#include "../include/gcn-con.h"
...
static struct console_data_s *default_console = (struct console_data_s *)-1;

static volatile unsigned long *vi_regs = (volatile unsigned long *)0xcc002000;

static unsigned long saved_video_regs[32] = { -1 };

static const unsigned long VIDEO_Mode640X480Pal60YUV16[32] = {
...
static void console_putc(struct console_data_s *con, char c)
{
	...
	flush_dcache_range(con->framebuffer,
			   (con->framebuffer + con->stride * con->yres));
}
			 ...
			 int xres, int yres, int stride)
{
	...
	con->font = fontdata_8x16;

	con->foreground = COLOR_BLACK;
	con->background = COLOR_WHITE;

	con->scrolled_lines = 0;

	c = (con->xres / 2) * con->yres;
...
static struct console_data_s gcn_con_data = { .foreground = 1 };
...
void gcn_con_init(void)
{
	unsigned long *video_mode_regs = (unsigned long *)&VIDEO_Mode640X480Pal60YUV16[0];
	int i;

	for(i = 0; i < 32; i++)
		saved_video_regs[i] = vi_regs[i];

        /* initialize video registers */
        for (i = 0; i < 7; i++) {
                vi_regs[i] = video_mode_regs[i];
        }
        vi_regs[8] = video_mode_regs[8];
        vi_regs[10] = video_mode_regs[10];
        vi_regs[11] = video_mode_regs[11];
        for (i = 16; i < 32; i++) {
                vi_regs[i] = video_mode_regs[i];
        }
	...
	writel(GCN_XFB_START, GCN_VI_TFBL);
	writel(GCN_XFB_START + 2 * 640, GCN_VI_BFBL);

	console_init(&gcn_con_data, (void *)(0x80000000 | GCN_XFB_START),
 ...
 *
 */
void gcn_con_exit(void)
{
	int i;

	for (i = 0; i < 32; i++)
		vi_regs[i] = saved_video_regs[i];
}
